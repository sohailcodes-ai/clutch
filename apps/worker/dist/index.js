...
import { EVALUATION_QUEUE_NAME, detectSubmissionSimilarity, evaluateAndAwardTitles, evaluateSubmission, markMatchEvaluating, recordSubmissionOutcome, resolveMatch, shouldEvaluateMatch, tryPairQueue, } from '@clutch/domain';
/**
 * ============================================================================
...
async function sweepExpiredMatches() {
    ...
    const expired = await db.query.matches.findMany({
        where: and(inArray(schema.matches.status, ['active', 'starting']), lte(schema.matches.endsAt, new Date())),
        ...
        try {
            ...
            for (const p of match.participants) {
                const final = await db.query.submissions.findFirst({
                    where: and(eq(schema.submissions.matchId, match.id), eq(schema.submissions.userId, p.userId), eq(schema.submissions.isFinal, true)),
                });
                if (!final) {
                    await db
...
const worker = new Worker(EVALUATION_QUEUE_NAME, async (job) => {

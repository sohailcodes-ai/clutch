...
function publicUser(user: SessionUser | PublicUser): PublicUser & { profile?: unknown } {
  return {
    id: user.id,
    email: user.email,
    status: user.status,
...
async function enforceRateLimit(
  ...
  opts: { key: string; limit: number; windowSec: number; failClosed?: boolean },
) {
...
export async function resolveWhenReady(db: Database, redis: Redis, matchId: string) {
  if (!(await shouldEvaluateMatch(db, matchId))) return false
...
export async function registerHttpRoutes(app: FastifyInstance) {
  ...
  app.get('/practice/questions', async (request) => {
    ...
    const rows = await request.server.db.query.questions.findMany({
      where: and(
        ...
        query.difficultyId ? eq(schema.questions.difficultyId, query.difficultyId) : undefined,
        query.topic ? eq(schema.questions.topic, query.topic) : undefined,
  ...
  app.get('/practice/recommendations', { preHandler: [requireAuth] }, async (request) => {
    ...
    const result = await recommendNextQuestions(
      request.server.db,
      request.user!.id,
      query.stackId,
      5,

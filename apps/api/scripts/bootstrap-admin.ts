...
import { createDb, closeDb } from '@clutch/db'
 ...
 *
 * Usage:
 *   CLUTCH_ADMIN_HANDLE=sami CLUTCH_ADMIN_PASSWORD=<secret> pnpm --filter @clutch/api bootstrap:admin
 *
...
async function main() {
  ...
  if (!handle || !password) {
    console.error(
      'CLUTCH_ADMIN_HANDLE and CLUTCH_ADMIN_PASSWORD environment variables are required',
    )
    process.exit(1)
  }

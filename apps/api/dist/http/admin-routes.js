...
import { z } from 'zod';
import { AppError, ErrorCodes, createQuestionSchema, createQuestionVersionSchema, addTestCasesSchema, updateQuestionMetaSchema, listAdminQuestionsQuerySchema, createEventSchema, createTournamentSchema, hasPermission, } from '@clutch/shared';
import { schema } from '@clutch/db';
import { createQuestionDraft, createQuestion, updateQuestionMeta, upsertDraftContent, addTestCasesToLatestVersion, publishQuestion, unpublishQuestion, archiveQuestion, listQuestionsForAdmin, getQuestionStats, createEvent, createTournament, seedRounds, writeAuditLog, getAdminOverview, listAdminMatches, inspectMatch, joinMatchAsObserver, leaveMatchObservation, adjudicateMatch, listAdminUsers, inspectAdminUser, setUserStatus, listAuditLog, listAbuseFlags, reviewAbuseFlag, isTitleCriteria, } from '@clutch/domain';
...
function adminOf(permission) {
    return [requireAuth, requirePermission(permission)];
}
const auditAction = async (app, userId, action, resourceType, resourceId, metadata = {}) => writeAuditLog(app.db, {
    actorUserId: userId,
    action,
    resourceType,
    resourceId,
...
export async function registerAdminRoutes(app) {
    ...
    app.post('/admin/matches/:matchId/adjudicate', { preHandler: adminOf('admin.matches.adjudicate') }, async (request) => {
        const { matchId } = parse(z.object({ matchId: z.string().uuid() }), request.params);
    ...
    app.get('/admin/moderation/flags', { preHandler: adminOf('admin.users.view') }, async (request) => {
        const query = parse(z.object({ status: z.enum(['open', 'reviewed', 'actioned', 'dismissed', 'all']).default('open') }), request.query);
    ...
    app.post('/admin/titles', { preHandler: adminOf('admin.titles.manage') }, async (request, reply) => {
        ...
        if (existing)
            throw new AppError(ErrorCodes.CONFLICT, 'Title code already exists', 409);

import { z } from 'zod';
import { AppError, ErrorCodes, createRoomSchema, joinRoomSchema, listRoomsQuerySchema, listEventsQuerySchema, } from '@clutch/shared';
import { getDashboard, getTitleCatalogForUser, equipTitle, listLiveMatches, listRecentResults, getSpectatorSnapshot, createRoom, joinRoom, leaveRoom, setRoomReady, getRoomDetail, listOpenRooms, startRoomMatch, getCurrentSeason, listEvents, getEvent, registerForEvent, unregisterForEvent, getEventStandings, listTournaments, getTournament, registerForTournament, unregisterForTournament, listParticipants, } from '@clutch/domain';
...
export async function registerDiscoveryRoutes(app) {
    // -------------------------------------------------------------------------
    ...
    app.post('/titles/equip', { preHandler: [requireAuth] }, async (request) => {
        const input = parse(z.object({ titleCode: z.string().min(1).max(64).nullable() }), request.body);
    ...
    app.get('/spectate/:publicId', async (request) => {
        const { publicId } = parse(z.object({ publicId: z.string().regex(/^CL-[0-9a-fA-F]{8}$/) }), request.params);
    ...
    app.post('/rooms/:roomId/start', { preHandler: [requireAuth] }, async (request, reply) => {
        ...
        if (!season)
            throw new AppError(ErrorCodes.CONFLICT, 'No active season', 409);
        const match = await startRoomMatch(app.db, app.redis, season.id, roomId, request.user.id);
        void reply.code(201);
    ...
    app.get('/tournaments/:slug', async (request) => {
        ...
        if (!tournament)
            throw new AppError(ErrorCodes.NOT_FOUND, 'Tournament not found', 404);
        return { tournament };
    });

...
export async function createSubmission(
  ...
  const outcome = await db.transaction(async (tx) => {
    ...
    const participant = await tx.query.matchParticipants.findFirst({
      where: and(
        eq(schema.matchParticipants.matchId, input.matchId),
        eq(schema.matchParticipants.userId, input.userId),
      ),
    })
    ...
    if (created) {
      ...
      return { created: true as const, submission: created }
    }

    const existing = await tx.query.submissions.findFirst({
      where: and(
        eq(schema.submissions.matchId, input.matchId),
        eq(schema.submissions.userId, input.userId),
        eq(schema.submissions.idempotencyKey, input.idempotencyKey),
      ),
    })
    if (!existing) throw new AppError(ErrorCodes.INTERNAL, 'Failed to create submission', 500)
    return { created: false as const, submission: existing }
  })
...
export async function shouldEvaluateMatch(db: Database, matchId: string) {
  const match = await db.query.matches.findFirst({
    where: eq(schema.matches.id, matchId),
    with: { participants: true },
  })
  if (!match || !(ACTIVE_MATCH_STATUSES as readonly string[]).includes(match.status)) return false

  if (match.endsAt && match.endsAt <= new Date()) return true
  ...
  const finals = await Promise.all(
    match.participants.map(async (p) => {
      ...
      return sub && (EVALUATED_STATUSES as readonly string[]).includes(sub.status) ? sub : null
    }),
  )
...
export async function markMatchEvaluating(db: Database, redis: Redis, matchId: string) {

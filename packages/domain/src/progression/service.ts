...
export function ladderIndex(bandId: string): number {
 ...
 * - Otherwise stay.
 */
export function decideProgression(signals: TopicSignal[], currentRating: number): ProgressionDecision {
  ...
  const ratingBand =
    DIFFICULTY_LADDER.find(
      (d) => currentRating >= d.minRating && currentRating <= d.maxRating,
    ) ?? DIFFICULTY_LADDER[0]!
...
export async function recordQuestionAttempt(
  db: DbExecutor,
  input: { userId: string; questionId: string; topic: string; difficultyId: string; passed: boolean; executionTimeMs?: number },
) {
  await db
 ...
 */
export async function recordSubmissionOutcome(db: DbExecutor, submissionId: string) {
...
export async function getUserProgress(db: DbExecutor, userId: string) {
...
export async function getTopicSignals(db: DbExecutor, userId: string): Promise<TopicSignal[]> {
  const rows = await db.query.userQuestionStats.findMany({
    where: eq(schema.userQuestionStats.userId, userId),
  })
...
export async function recommendNextQuestions(
  ...
  let candidates = await db.query.questions.findMany({
    where: and(
      ...
      eq(schema.questions.difficultyId, decision.targetBandId),
      supported.length > 0 ? inArray(schema.questions.id, supported) : undefined,
    ),
    limit: limit * 4,
  })

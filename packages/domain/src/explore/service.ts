...
export async function listLiveMatches(db: Database, limit = 10) {
...
export async function listRecentResults(db: Database, limit = 10) {
  const rows = await db.query.matches.findMany({
 ...
 */
export async function getSpectatorSnapshot(db: Database, publicId: string) {
  ...
  const totalWeight =
    match.questionVersion.testCases.reduce((sum, t) => sum + t.weight, 0) || 1

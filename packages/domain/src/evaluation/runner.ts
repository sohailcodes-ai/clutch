...
export async function evaluateSubmission(db: Database, submissionId: string) {
  ...
  const finalStatus: SubmissionStatus =
    passed >= tests.reduce((sum, t) => sum + (t.weight ?? 1), 0) ? 'accepted' : 'wrong_answer'
...
function runReferenceSolution(sourceCode: string, language: string, input: string): string {
  void language
  void sourceCode
  ...
  if (sorted.length % 2 === 1) return String(sorted[mid])

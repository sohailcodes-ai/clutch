...
import { pairingInitialBand, placementTargetShift } from '../rating/placement.js'
...
const ACTIVE_MATCH_STATUSES = ['matched', 'starting', 'active', 'evaluating'] as const

function queueKey(seasonId: string, stackId: string) {
  return `queue:${seasonId}:${stackId}`
}

function queueMetaKey(entryId: string) {
  return `queue:meta:${entryId}`
}

function pairLockKey(seasonId: string, stackId: string) {
  return `lock:pair:${seasonId}:${stackId}`
}

function recentPairKey(a: string, b: string) {
  const sorted = [a, b].sort()
  return `recent_pair:${sorted[0]}:${sorted[1]}`
}

export function ratingBucket(rating: number, bandSize: number = QUEUE_BAND_INITIAL) {
  return Math.floor(rating / bandSize) * bandSize
}

export function expandedBand(
  baseBucket: number,
  waitSeconds: number,
  initialBand: number = QUEUE_BAND_INITIAL,
) {
...
async function findActiveMatch(db: Database, userId: string) {
  const rows = await db.query.matchParticipants.findMany({
    where: eq(schema.matchParticipants.userId, userId),
    with: { match: true },
  })
  return rows.find((row) =>
    (ACTIVE_MATCH_STATUSES as readonly string[]).includes(row.match.status),
  )
}
...
export async function joinQueue(
  db: Database,
  redis: Redis,
  input: { userId: string; stackId: string; difficultyId?: string },
) {
  ...
  if (!season) throw new AppError(ErrorCodes.INTERNAL, 'No active season', 500)

  const ratingRow = await db.query.userStackRatings.findFirst({
    where: and(
      eq(schema.userStackRatings.userId, input.userId),
      eq(schema.userStackRatings.stackId, input.stackId),
    ),
  })
  if (!ratingRow) throw new AppError(ErrorCodes.NOT_FOUND, 'Stack rating not found', 404)
...
export async function leaveQueue(db: Database, redis: Redis, userId: string) {
  ...
  if (!entry) throw new AppError(ErrorCodes.NOT_IN_QUEUE, 'Not in queue', 404)

  const cancelled = await db
    .update(schema.queueEntries)
    .set({ status: 'cancelled' })
    .where(
      and(eq(schema.queueEntries.id, entry.id), eq(schema.queueEntries.status, 'waiting')),
    )
 ...
 */
export async function tryPairQueue(
  db: Database,
  redis: Redis,
  seasonId: string,
  stackId: string,
  ...
  try {
    ...
    const metas: QueueMeta[] = await Promise.all(
    ...
    for (let i = 0; i < metas.length; i++) {
      for (let j = i + 1; j < metas.length; j++) {
        const a = metas[i]!
        const b = metas[j]!
        if (!a.userId || !b.userId || !a.rating || !b.rating || !a.enqueuedAt || !b.enqueuedAt) continue

        // Re-verify both entries against PostgreSQL before pairing.
        const freshEntries = await db.query.queueEntries.findMany({
          where: and(
            inArray(schema.queueEntries.id, [a.id, b.id]),
        ...
        const involvesPlacement =
          placementRemainingA > 0 || placementRemainingB > 0
        const band = expandedBand(
          ratingBucket(Number(a.rating)),
          waitSeconds,
          pairingInitialBand(involvesPlacement),
        )
        const rb = Number(b.rating)
        if (rb < band.min || rb > band.max) continue

        const pairKey = recentPairKey(a.userId, b.userId)
        if (await redis.exists(pairKey)) continue
        ...
        const sharedDifficulty =
          entryA?.difficultyId && entryA.difficultyId === entryB?.difficultyId
            ? entryA.difficultyId
            : null
        const selected = await selectQuestionForMatch(
          db,
          stackId,
          (Number(a.rating) + rb) / 2,
          [a.userId, b.userId],
          {
        ...
        const ratings = await db.query.userStackRatings.findMany({
          where: and(
            inArray(schema.userStackRatings.userId, [a.userId, b.userId]),
        ...
        const match = await db.transaction(async (tx) => {
          ...
          if (claimed.length !== 2) throw new AppError(ErrorCodes.CONFLICT, 'Queue entry changed', 409)
          ...
          if (err instanceof AppError && err.code === ErrorCodes.CONFLICT) return null
          throw err
        })
...
export async function getQueueStatus(db: Database, userId: string) {

 ...
 */
export function bandWalkOrder(sortOrders: number[], targetSortOrder: number): number[] {
  ...
  const order: number[] = []
  if (unique.includes(targetSortOrder)) order.push(targetSortOrder)
  const maxLen = Math.max(lower.length, upper.length)
  for (let i = 0; i < maxLen; i++) {
    const up = upper[i]
    const low = lower[i]
    if (up !== undefined) order.push(up)
    if (low !== undefined) order.push(low)
  }
  return order
}
 ...
 */
export function pairJitter(seedParts: string[], slug: string): number {
  let h = 0x811c9dc5
 ...
 */
export function scoreCandidate(c: CandidateScoreInput, jitterSeed: string[]): number {
...
export function pickBest<T extends { slug: string; timesSeen: number; bandSortOrder: number }>(
  pool: readonly T[],
  targetSortOrder: number,
  jitterSeed: string[],
): T | null {
  ...
  let bestScore = scoreCandidate(
    { slug: best.slug, bandSortOrder: best.bandSortOrder, targetSortOrder, timesSeen: best.timesSeen },
    jitterSeed,
  )
  for (const c of pool.slice(1)) {
    const s = scoreCandidate(
      { slug: c.slug, bandSortOrder: c.bandSortOrder, targetSortOrder, timesSeen: c.timesSeen },
      jitterSeed,
    )
    if (s > bestScore) {
      best = c
...
async function assertStacksExist(db: DbExecutor, stackIds: string[]) {
...
async function assertBandExists(db: DbExecutor, difficultyId: string) {
...
export async function createQuestionDraft(db: Database, input: CreateQuestionInput) {
  ...
  return db.transaction(async (tx) => {
    ...
    if (!q) throw new AppError(ErrorCodes.INTERNAL, 'Failed to create question', 500)
    ...
    if (!version) throw new AppError(ErrorCodes.INTERNAL, 'Failed to create question version', 500)
    ...
    await tx.insert(schema.testCases).values(
      input.testCases.map((test, index) => ({
...
export async function createQuestion(db: Database, input: CreateQuestionInput) {
  await assertBandExists(db, input.difficultyId)
  await assertStacksExist(db, input.stackIds)
...
async function loadQuestionOrThrow(db: DbExecutor, questionId: string) {
  const q = await db.query.questions.findFirst({
    where: eq(schema.questions.id, questionId),
    with: { versions: true },
  })
  if (!q) throw new AppError(ErrorCodes.NOT_FOUND, 'Question not found', 404)
  return q
}

function latestVersion<T extends { version: number }>(versions: T[]): T | undefined {
...
export async function updateQuestionMeta(
  ...
  if (input.difficultyId) await assertBandExists(db, input.difficultyId)
  if (input.stackIds) await assertStacksExist(db, input.stackIds)
 ...
 */
export async function upsertDraftContent(
  db: Database,
  questionId: string,
  content: CreateQuestionVersionInput,
) {
  const q = await loadQuestionOrThrow(db, questionId)
  const latest = latestVersion(q.versions)

  if (!latest) throw new AppError(ErrorCodes.INTERNAL, 'Question has no versions', 500)
  ...
  if (q.status === 'draft') {
    const [updated] = await db
      ...
      .where(
        and(
          eq(schema.questionVersions.id, latest.id),
          sql`${schema.questionVersions.publishedAt} IS NULL`,
        ),
      )
      .returning()
    if (!updated) throw new AppError(ErrorCodes.CONFLICT, 'Version already published', 409)
    return updated
  }
  ...
  if (!created) throw new AppError(ErrorCodes.INTERNAL, 'Failed to create version', 500)
  return created
}
...
export async function addTestCasesToLatestVersion(
  db: Database,
  questionId: string,
  tests: AddTestCaseRow[],
) {
  const q = await loadQuestionOrThrow(db, questionId)
  const latest = latestVersion(q.versions)
  if (!latest) throw new AppError(ErrorCodes.INTERNAL, 'Question has no versions', 500)
  if (latest.publishedAt && q.status === 'published') {
    ...
    throw new AppError(
      ErrorCodes.CONFLICT,
      'Cannot modify tests of a published version; create a new version',
      409,
    )
  }
  const maxOrdinal =
...
export async function publishQuestion(db: Database, questionId: string) {
  const q = await loadQuestionOrThrow(db, questionId)
  const latest = latestVersion(q.versions)
  if (!latest) throw new AppError(ErrorCodes.INTERNAL, 'Question has no versions', 500)
...
export async function unpublishQuestion(db: Database, questionId: string) {
  const [updated] = await db
    .update(schema.questions)
    .set({ status: 'draft' })
    .where(and(eq(schema.questions.id, questionId), neStatus('retired')))
    .returning()
  if (!updated) throw new AppError(ErrorCodes.CONFLICT, 'Question cannot be unpublished', 409)
  return updated
}

export async function archiveQuestion(db: Database, questionId: string) {
  ...
  const [updated] = await db
    ...
    .where(eq(schema.questions.id, questionId))
    .returning()
  if (!updated) throw new AppError(ErrorCodes.NOT_FOUND, 'Question not found', 404)
  return updated
}

function neStatus(status: 'retired') {
  return sql`${schema.questions.status} <> ${status}`
}

export async function listQuestionsForAdmin(
  db: DbExecutor,
  opts: { status?: 'draft' | 'published' | 'retired' | 'all'; limit: number; offset: number },
) {
  const where =
...
export async function getQuestionStats(db: DbExecutor, questionId: string) {
  ...
  if (versionIds.length === 0) {
    return {
      ...
      passRate: 0,
      averageSolveTimeMs: null as number | null,
      distinctSolvers: 0,
    }
  }
  ...
  const [subStats] = await db
    .select({
      ...
      accepted: sql<number>`COUNT(*) FILTER (WHERE ${schema.submissions.status} = 'accepted')`,
      avgMs: sql<number | null>`AVG(${schema.submissions.executionTimeMs}) FILTER (WHERE ${schema.submissions.isFinal} = TRUE)`,
...
// ---------------------------------------------------------------------------
// Match question selection — stack-aware, adaptive, deadlock-free
// ---------------------------------------------------------------------------
...
export function chooseTargetBandIndex(
  bandCount: number,
  baseIndex: number,
  userAccuracies: number[],
): number {
  let idx = baseIndex
...
export async function selectQuestionForMatch(
  db: Database,
  stackId: string,
  avgRating: number,
  ...
  const preferredBand =
    options.preferredDifficultyId != null
  ...
  const history = await db.query.userQuestionHistory.findMany({
    where: and(
      inArray(schema.userQuestionHistory.userId, userIds),
      inArray(schema.userQuestionHistory.questionId, questionIds),
    ),
  })
  const timesSeenByQuestion = new Map<string, number>()
  ...
  const walk = bandWalkOrder(
    selectable.map((q) => bands.find((b) => b.id === q.difficultyId)?.sortOrder ?? targetBand.sortOrder),
    targetBand.sortOrder,
  )

  let pool: typeof selectable = []
  let resolvedTargetSortOrder = targetBand.sortOrder
  for (const sortOrder of walk) {
    const inBand = selectable.filter(
      (q) => bands.find((b) => b.id === q.difficultyId)?.sortOrder === sortOrder,
    )
    if (inBand.length > 0) {
      pool = inBand
  ...
  if (pool.length === 0) pool = selectable
  ...
  const version =
    picked.versions.filter((v) => v.publishedAt !== null).sort((a, b) => a.version - b.version).at(-1) ??
    undefined
...
export async function getQuestionForEditor(db: Database, questionVersionId: string) {

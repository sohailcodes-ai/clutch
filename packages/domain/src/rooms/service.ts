...
export async function createRoom(db: Database, hostUserId: string, input: CreateRoomInput) {
  ...
  return db.transaction(async (tx) => {
    const [room] = await tx
      ...
      .returning()
    if (!room) throw new AppError(ErrorCodes.INTERNAL, 'Failed to create room', 500)
...
export async function leaveRoom(db: Database, roomId: string, userId: string) {
  const room = await db.query.rooms.findFirst({ where: eq(schema.rooms.id, roomId) })
  if (!room) throw new AppError(ErrorCodes.NOT_FOUND, 'Room not found', 404)
...
export async function setRoomReady(db: Database, roomId: string, userId: string, ready: boolean) {
...
export async function startRoomMatch(
  ...
  const selected = await selectQuestionForMatch(
    db,
    room.stackId,
    (ratingOf(first.userId) + ratingOf(second.userId)) / 2,

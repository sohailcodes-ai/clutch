...
export function deriveAdminEmail(handle: string): string {
...
export async function bootstrapAdmin(
...
): Promise<{ created: boolean; promoted: boolean; userId: string }> {
  ...
  const userId = await db.transaction(async (tx) => {
    const [created] = await tx
      ...
      .returning()
    if (!created) throw new AppError(ErrorCodes.INTERNAL, 'Failed to create admin user', 500)

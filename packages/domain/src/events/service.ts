...
async function assertEventConfig(db: Database, input: CreateEventInput) {
...
export async function createEvent(db: Database, input: CreateEventInput) {
  ...
  return db.transaction(async (tx) => {
    const [event] = await tx
      ...
      .returning()
    if (!event) throw new AppError(ErrorCodes.INTERNAL, 'Failed to create event', 500)

    await tx
      .insert(schema.eventStacks)
      .values(input.stackIds.map((stackId) => ({ eventId: event.id, stackId })))
    await tx.insert(schema.eventDifficultyLevels).values(
      input.allowedDifficultyIds.map((difficultyId) => ({ eventId: event.id, difficultyId })),
    )
    return event
  })
}
...
export async function listEvents(
  db: Database,
  opts: { phase?: 'upcoming' | 'active' | 'ended' | 'all'; limit?: number; offset?: number } = {},
) {
  const now = new Date()
...
export async function getEvent(db: Database, slug: string) {
  const now = new Date()
...
export async function registerForEvent(db: Database, slug: string, userId: string) {
  const now = new Date()
  ...
  if (!window.ok) throw new AppError(ErrorCodes.VALIDATION, window.reason, 409)
...
export async function unregisterForEvent(db: Database, slug: string, userId: string) {
  const event = await db.query.events.findFirst({ where: eq(schema.events.slug, slug) })
...
export async function getEventStandings(db: Database, slug: string, limit = 20) {
  const event = await db.query.events.findFirst({ where: eq(schema.events.slug, slug) })
  if (!event) throw new AppError(ErrorCodes.NOT_FOUND, 'Event not found', 404)
  ...
  const scored = new Map<string, { handle: string | null; avatarUrl: string | null; points: number; wins: number; games: number }>()
  for (const p of participants) {
    if (p.match.eventId !== event.id) continue
    if (!['resolved', 'draw', 'abandoned'].includes(p.match.status)) continue

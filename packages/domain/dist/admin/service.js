 ...
 */
export function redactSubmissionForAdmin(s) {
    return {
        id: s.id,
        userId: s.userId,
        status: s.status,
...
export async function getAdminOverview(db) {
    const [questionCounts] = await db
        .select({
        published: sql `COUNT(*) FILTER (WHERE ${schema.questions.status} = 'published')`,
        drafts: sql `COUNT(*) FILTER (WHERE ${schema.questions.status} = 'draft')`,
        archived: sql `COUNT(*) FILTER (WHERE ${schema.questions.status} = 'retired')`,
    })
    ...
    const [flagCount] = await db
        .select({ open: sql `COUNT(*) FILTER (WHERE ${schema.abuseFlags.status} = 'open')` })
...
export async function listAdminMatches(db, limit = 25) {
    ...
    const progress = await db
        ...
        attempts: sql `COUNT(*)`,
        evaluating: sql `COUNT(*) FILTER (WHERE ${schema.submissions.status} IN ('queued','running'))`,
    })
    ...
    return rows.map((m) => ({
        ...
        publicId: m.publicId,
        status: m.status,
        phase: m.status === 'active' || m.status === 'evaluating'
            ? 'in_duel'
            : 'lobby',
        stackId: m.stackId,
        ...
        endsAt: m.endsAt?.toISOString() ?? null,
        remainingSec: m.endsAt !== null ? Math.max(0, Math.round((m.endsAt.getTime() - now.getTime()) / 1000)) : null,
        serverTimeMs: now.getTime(),
...
export async function inspectMatch(db, matchId) {
    ...
    const ratings = await db.query.userStackRatings.findMany({
        where: and(inArray(schema.userStackRatings.userId, m.participants.map((p) => p.userId)), eq(schema.userStackRatings.stackId, m.stackId)),
    });
    ...
    return {
        ...
        publicId: m.publicId,
        status: m.status,
        resolutionLabel: m.resolveReason === 'adjudicated'
            ? 'admin_adjudication'
...
// ---------------------------------------------------------------------------
async function loadLiveMatchOrThrow(db, matchId) {
 ...
 */
export async function joinMatchAsObserver(db, redis, matchId, admin) {
...
export async function leaveMatchObservation(db, redis, matchId, admin) {
    const match = await db.query.matches.findFirst({ where: eq(schema.matches.id, matchId) });
    if (!match)
        throw new AppError(ErrorCodes.NOT_FOUND, 'Match not found', 404);
    if (!(await hasActiveObservation(db, matchId, admin.userId))) {
 ...
 */
export async function hasActiveObservation(db, matchId, adminUserId) {
    const events = await db.query.matchEvents.findMany({
        where: and(eq(schema.matchEvents.matchId, matchId), eq(schema.matchEvents.actorUserId, adminUserId), inArray(schema.matchEvents.eventType, ['admin.joined', 'admin.left'])),
...
// ---------------------------------------------------------------------------
export async function listAdminUsers(db, query, limit = 50) {
    ...
    return profiles.map((p) => {
        ...
        const best = userRatings.reduce((top, r) => (!top || r.rating > top.rating ? r : top), null);
 ...
 */
export async function inspectAdminUser(db, userId, opts) {
...
export async function setUserStatus(db, adminUserId, userId, status) {
...
export async function listAuditLog(db, opts) {
    ...
    const adminIds = [...new Set(entries.map((e) => e.actorUserId).filter((v) => v !== null))];
...
// ---------------------------------------------------------------------------
export async function listAbuseFlags(db, status = 'open', limit = 50) {
    const rows = await db.query.abuseFlags.findMany({
        ...(status === 'all' ? {} : { where: eq(schema.abuseFlags.status, status) }),
    ...
    return rows.map((f) => ({
        ...
        evidenceSummary: typeof f.evidence === 'object' && f.evidence !== null && 'similarity' in f.evidence
            ? { similarity: f.evidence.similarity ?? null }
            : {},
    }));
}
export async function reviewAbuseFlag(db, adminUserId, flagId, decision) {
    const flag = await db.query.abuseFlags.findFirst({ where: eq(schema.abuseFlags.id, flagId) });
    if (!flag)
        throw new AppError(ErrorCodes.NOT_FOUND, 'Flag not found', 404);
    const [updated] = await db

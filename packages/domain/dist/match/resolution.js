...
const ADJUDICABLE_STATUSES = ['matched', 'starting', 'active', 'evaluating', 'abandoned'];
/**
 ...
 */
export function validateAdjudication(candidate, input) {
    ...
    if (!ADJUDICABLE_STATUSES.includes(candidate.status)) {
 ...
 */
export async function adjudicateMatch(db, redis, input) {
    const existing = await db.query.matches.findFirst({
    ...
    if (!loser)
        throw new AppError(ErrorCodes.CONFLICT, 'Loser could not be determined', 409);
    const totalWeight = (await db.query.testCases.findMany({
    ...
    const winnerResult = {
        ...
        ratingBefore: existing.participants.find((p) => p.userId === input.winnerUserId)?.ratingBefore ?? 1000,
    ...
    const final = (await db.query.matches.findFirst({ where: eq(schema.matches.id, existing.id) })) ?? null;
    if (committed && final) {
        for (const p of existing.participants) {
            await publishUserEvent(redis, p.userId, {
                type: 'match.resolved',
    ...
    if (!final)
        throw new AppError(ErrorCodes.INTERNAL, 'Failed to re-read adjudicated match', 500);
    return final;
}

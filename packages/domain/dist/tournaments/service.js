...
import { AppError, ErrorCodes, canRegisterForTournament, } from '@clutch/shared';
/**
...
export async function createTournament(db, input) {
    if (input.registrationClosesAt > input.startsAt) {
        throw new AppError(ErrorCodes.VALIDATION, 'Registration must close before the tournament starts', 400);
    }
...
export async function listTournaments(db, limit = 20, offset = 0) {
    const now = new Date();
...
export async function getTournament(db, slug) {
    const now = new Date();
...
export async function registerForTournament(db, slug, userId) {
    const now = new Date();
    ...
    if (!check.ok)
        throw new AppError(ErrorCodes.VALIDATION, check.reason, 409);
    const existing = await db.query.tournamentRegistrations.findFirst({
        where: and(eq(schema.tournamentRegistrations.tournamentId, tournament.id), eq(schema.tournamentRegistrations.userId, userId)),
    });
    if (existing)
...
export async function unregisterForTournament(db, slug, userId) {
    const tournament = await db.query.tournaments.findFirst({
        where: eq(schema.tournaments.slug, slug),
    });
    if (!tournament)
...
export async function listParticipants(db, slug) {
    const tournament = await db.query.tournaments.findFirst({
        where: eq(schema.tournaments.slug, slug),
    });
    if (!tournament)
...
export async function seedRounds(db, slug, roundNames = ['Quarterfinals', 'Semifinals', 'Final']) {
    const tournament = await db.query.tournaments.findFirst({
        where: eq(schema.tournaments.slug, slug),
    });
    if (!tournament)
    ...
    await db.transaction(async (tx) => {
        ...
        await tx.insert(schema.tournamentRounds).values(roundNames.map((name, i) => ({

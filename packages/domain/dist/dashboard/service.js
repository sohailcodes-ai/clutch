 ...
 */
export async function getPlayerCard(db, userId) {
 ...
 */
export async function getRecentMatches(db, userId, limit = 10) {
    ...
    const opponentUserIds = finished.flatMap((p) => p.match.participants.filter((x) => x.userId !== userId).map((x) => x.userId));
    ...
    return finished.map((p) => {
        ...
        return sanitizeRecentMatch({
            ...
            result: m.status === 'draw' && !m.winnerUserId
                ? 'draw'
...
function startEndDuration(start, end) {
    if (!start || !end)
        return null;

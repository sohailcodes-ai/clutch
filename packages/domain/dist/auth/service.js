...
export async function registerUser(db, input, meta) {
    ...
    const user = await db.transaction(async (tx) => {
        const [created] = await tx
            ...
            .returning();
        if (!created)
            throw new AppError(ErrorCodes.INTERNAL, 'Failed to create user', 500);
        try {
...
export async function loginUser(db, input, meta) {
...
export async function getSessionUser(db, token) {
    const session = await db.query.authSessions.findFirst({
        where: and(eq(schema.authSessions.tokenHash, hashToken(token)), gt(schema.authSessions.expiresAt, new Date())),
        with: {
            user: {
                with: { profile: true },
            },
